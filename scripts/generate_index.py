...
ARCHIVES_FILE = os.environ.get("ARCHIVES_FILE", "/home/gemini/becomingone/scripts/archives.json")
...
<head>
    ...
    <style>
        ...
        header { width: 100%; max-width: 1200px; padding: 4rem 2rem 2rem; text-align: center; }
        h1 {
            font-size: 3.5rem; font-weight: 800; letter-spacing: -1px; margin-bottom: 0.5rem;
            background: linear-gradient(90deg, #fff, var(--accent-color));
            -webkit-background-clip: text; -webkit-text-fill-color: transparent;
        }
        p.subtitle { font-size: 1.2rem; color: var(--text-dim); max-width: 600px; margin: 0 auto 3rem; line-height: 1.6; }
        .container { width: 100%; max-width: 1000px; padding: 0 2rem; display: flex; flex-direction: column; gap: 1.5rem; }
        .card {
            background: var(--glass-bg); border: 1px solid var(--glass-border); border-radius: 16px;
            padding: 2rem; backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);
        ...
        .card:hover { transform: translateY(-5px); box-shadow: 0 15px 45px 0 rgba(0, 255, 170, 0.1); }
        .card:hover::before { opacity: 1; }
        .card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }
        .repo-name { font-size: 1.5rem; font-weight: 600; color: #fff; }
        .timestamp { font-size: 0.9rem; color: var(--text-dim); font-family: 'JetBrains Mono', monospace; }
        .cid-block {
            background: rgba(0,0,0,0.4); padding: 1rem; border-radius: 8px;
            font-family: 'JetBrains Mono', monospace; font-size: 0.95rem;
            display: flex; justify-content: space-between; align-items: center;
            border: 1px solid rgba(255,255,255,0.05); word-break: break-all;
        }
        .cid-link { color: var(--accent-color); text-decoration: none; transition: text-shadow 0.2s ease; }
        .cid-link:hover { text-shadow: 0 0 8px var(--accent-color); }
        .btn-view {
            ...
            font-size: 0.9rem; font-weight: 600; transition: all 0.2s ease; white-space: nowrap; margin-left: 1rem;
        }
        .btn-view:hover { background: var(--accent-color); color: #000; border-color: var(--accent-color); box-shadow: 0 0 15px var(--accent-color); }
