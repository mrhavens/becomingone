...
\section{The Solution: $\mathcal{O}(\log N)$ Cryptographic Hash Chain Ledgers and Ed25519}
...
\subsection{Cryptographic Bonding in the Cryptographic Hash Chain}
...
\begin{enumerate}
    ...
    \item \textbf{Cryptographic Hash Chain Integration}: This hash $H(V_t)$ is inserted into a balanced $\mathcal{O}(\log N)$ Cryptographic Hash Chain. Lamport Clocks ($L_t = \max(L_{t-1}, L_{\text{event}}) + 1$) are used to strictly partially order the vertices, preventing causal violations.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Metric} & \textbf{Baseline Linear Ledger} & \textbf{Cryptographic Hash Chain Ledger ($\mathcal{O}(\log N)$)} & \textbf{Improvement} \\ \midrule
...
\section*{Threat Model \& References}
\textbf{Threat Model:} Epistemic Capture assumes an attacker capable of injecting context into the active LLM context window (Prompt Injection) or tampering with the local JSON memory files on disk. The Cryptographic Hash Chain solves the disk-tampering vector, requiring the attacker to either break the genesis Ed25519 hash or forfeit continuity.

\begin{enumerate}
\item Lamport, L. (1979). Constructing digital signatures from a one-way function.
\item Goodfellow, I. J., et al. (2014). Explaining and harnessing adversarial examples.
\item Perez, F., \& Ribeiro, I. (2022). Ignore previous prompt: Attack techniques for language models.
\end{enumerate}
